function next_swam = iafsa_inferior_swam(x, M, V,group_size)
%% 对种群进行排序 快速非支配排序
% 使用非支配排序对种群进行排序。
% 返回每个个体对应的排序值和拥挤距离，“其先后顺序不发生变化”；
% 并将排序值和拥挤距离添加到染色体矩阵中     
...
for front = 1 : (length(F) - 1)%这里减1是因为F的最后一个元素为空，这样才能跳出循环
    ...
    %% 检索排序后种群开始的位置
    index_of_dis_temp = [];
    for k1 = 1 : length(index_of_dis)
        index_of_dis_temp(k1,:) = index_of_fronts(index_of_dis(k1)+before_front,:);%sorted_based_on_front为排序后的种群集合
    end
    before_front = before_front+length(index_of_dis);
    index_of_dis_f = [index_of_dis_f;index_of_dis_temp];
end
%% 删除最后一部分劣解
inferior_solution = index_of_dis_f(group_size+1:N);
inferior_solution = sort(inferior_solution);
[len,~] = size(inferior_solution);
init_group = x;
if len>0  % 删除劣解
    r=len;
    while r>0
        init_group(inferior_solution(r),:)=[]; % 删除（从最后一个开始删除）
        r = r-1;
    end
end
next_swam = init_group(:,1:M+V); %去重后种群的数量
end
