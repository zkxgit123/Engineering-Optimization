function [Optimum_seq_fpa] = Algorithm_FPA(kk1,init_pop,M,V,Num_ite,group_size,P_poll,Ref_point,Buf,Obj)
%% 初始种群非支配排序
...
%% FPA迭代优化
for ite=2:Num_ite
    fprintf('正在进行FPA算法第%d次运算,第%d次迭代,',kk1,ite);
    tic
    Parent_sequence = seq_iteration_fpa{ite-1,1};   % 前一步父代种群
    Children_sequence = FPA_pollination(Parent_sequence,P_poll,Buf);% 输出49个优化种群
