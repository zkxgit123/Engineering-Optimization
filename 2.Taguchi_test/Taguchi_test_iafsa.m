...
for i1=1:Operat_Times
    ...
    for j1=1:row_test
        ...
        ans_set = Algorithm_IAFSA(j1,init_pop,Num_ite,group_size,p_swarm,delta,visual_range,Ref_point,Buf,Obj);  % IAFSA算法
        final_seq{i1,j1} = ans_set(:,:); % 算法Alg_num运算kk1次之后存放的位置
    end
end
Taguchi = zeros(row_test,Operat_Times);
for i1=1:Operat_Times
    for j1=1:row_test
        ans_set =  final_seq{i1,j1};
        Taguchi(j1,i1) = cell2mat(ans_set(Num_ite,2));
    end
end
%% 算法迭代效果分析
% save('Taguchi.mat','Taguchi');
